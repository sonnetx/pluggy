...
| `parallelism/fsdp2.py` | **v0 correctness done (8/8, phase 2.2).** per-param Shard(0) over plain tensors, block + root units (root stays gathered through the loss: the fused-ce/lm_head fix), storage-resize resharding, reduce-scatter-every-microbatch accumulation, optimizer state at shard shapes. parity + memory invariants green on gloo at 2/4 ranks (`tests/fsdp2.py`, in ci). composes with per-block `torch.compile` as of 8/8 — the hooks are `torch._dynamo.disable`d, since compiling `_call_impl` used to trace the unshard's `p.data = ...` into the graph and AOTAutograd rejected it (CHANGES round 3: +36% tps, −26% peak mem on 4x H100; `test_compile_parity`). still ahead: comm stream + prefetch + flat per-unit gathers (perf), bf16-on-gather policy, gpu scaling numbers |
| `parallelism/{context_parallel,expert_parallel}.py` | empty stubs |
| models | qwen3 dense (0.6B validated, per-block compile, ~13k tps on A40, 73.5k global on 8x A40); llama3 stubbed |
| trainer | single-gpu and dp through the same path (world_size=1 is not a special case). lifecycle build → parallelize → init → optimizer → data, so sharded init slots in without reordering. grad accumulation (`global_batch_size / (micro_batch_size × dp)` microbatches) and grad-norm clipping both land in `train_step`; `train_step` returns a metrics dict (loss, grad_norm). no eval loop, no metrics logger. parallelism is config-selected as of 8/8 (`"parallelism": "ddp" \| "fsdp2"` -> `self.dp`, both driving the same requires_sync/sync surface; clip passes `shard_dims=("dp",)` under fsdp2; fsdp2 is benchmark-only until phase 3's sharded checkpointing) |
...
| dataloader | streaming HF + packing (stream + best-fit-decreasing) + stateful + CUDA prefetcher; splits by the mesh's dp coordinate. packing allows cross-document attention (no doc mask); prefetcher resume is exact (state snapshot taken before each pull). multi-source mixing as of 8/8: `data.sources` with weights that are **token** shares (each source is packed before the interleave, so a row is seq_len tokens of one corpus), per-rank source draws, mixture rng inside the checkpointed loader state; ratios/purity/resume tested in `tests/data_mixing.py` (cpu, in ci) |
